...
const CITIES: City[] = [
  { name: "Atlanta, GA", tag: "In-person & remote", slug: "atlanta" },
  { name: "Miami, FL", tag: "In-person & remote", slug: "miami" },
  { name: "Kingston, Jamaica", tag: "In-person & remote", slug: "kingston" },
    ...
    <section id="areas" className="relative py-24 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CITIES.map((city, i) => {
            const href = city.slug ? `/deploy/${city.slug}` : CALENDLY_URL;
                ...
                <span
                  className={`text-[11px] px-2.5 py-1 rounded-full ${
                    city.tag === "In-person & remote"
                      ? "text-accent-light/80 bg-accent/10 border border-accent/20"
